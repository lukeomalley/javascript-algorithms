 ...
 * and you want to reward your students. You decide to do so fairly by giving them
 * arbitrary rewards following two rules: first, all students must receive at least one reward;
 ...
 * strictly fewer rewards than an adjacent student with a higher score. Assume that all students
 * have different scores; in other words, the scores are all unique. Write a function that takes
 * in a list of scores and returns the minimum number of rewards that you must give out to students,
...
const minRewards = scores => {
  ...
  return rewards.reduce((acc, num) => acc + num);
};
